...
pageid: lang-js
keywords: !variable $car, 'input' 'mousex', 'mousey' 'something' **not** *global +amount; +data -1` 0 0, 0; 0` 1 100% 1; 1] 2 3 3rd 4 5 6 80 @class @param @property @return @typedef [a [amount [angular] [dizzy] [eslint] [fid-umd] [general [git][topic-git] [markdown][lang-md] [message] [modulejs] [opentoken] [restify `!!bool` `!!~haystack `$elements `+num `+num` `0` `5 `@param` `@return` `arguments` `array` `boolean` `break;` `camelcase` `case` `const` `copyproperty` `date` `datepicker-directive `default` `eventemitter` `false` `haystack `if `if` `lib `module `nan` `nan`, `null` `null`, `number` `promise`, `response-template-service `response-template-service-declined `return` `some-directive-service `spec `string` `switch` `this` `undefined` `undefined`, `var` `wind` `window` `xxxx[]` a abc-manager about above access accesses according achieve add added adds after aim alert aliases all alongside alphabetical alphabetically already also always amd amount an and angular angular, angular-specific annotations anonymous anotherclass~speciallyformattedobject any anything api app apply are args args; arguments arrange array arrays arrayvar arrow as assign assigned assignments assigns associated asterisks at attached automate avoid away back, basic be because becomes been before beginning behavior, behavioral between bigger bit bits blank block blocks bodies body boilerplate boolean boolean, braces brackets branch branches browser browser, browsers bus, but by call callcount callcount, callcount: calling calls can capital capitalization capitalized car, car; care careful carefully case cases cast catch certain chained chaining change charactercount charactercount; characters check checker checking checks class class, classes cleanliness, closely code code, codes, collection colon, column come comma commas comment commented comments comments, compare comparison compelling compulsory concat configuration conflict conform considered consistency consistent console console, constants constants, constructor constructors constructs consumed contains content contents context control copy copyproperty correct could count counter coverage create creates css` current custom data data[name] data[name]; datatypes, debuggable declaration declaration, declared declaring define defining dependencies dependency describe describes description descriptions designed desired development different directives, directly directories disable display displayable do documented documenting does don't double-quoted duck each early easier easy ecma eg either element else embedding empty enable encourage encouraged end ending enforced entire entirely entirely; enumerated environment error es5 es6 es6, eslint eslintignore eslintignore] eslintrc eslintrc-angular etc even event event, every examine example example, examples, exception exceptions executes expanded expected explain explaining export exports exports[name] exports` exposed extend factory factory, fake faketestclass faketestclass; faketestdata false false, falsy far few fid-umd file file's filename files find first first; fit flags folder folders follow followed foobar for forced forces foreach formatted formatting former four from front front, function function, functions functions, general general, get gets global global, globals goes going grammatically guide handler handling has have header help helps here here] hoisting however however, html html` if illustrate illustration illustrative immediately immediatly in include includes incoming increases increment increments indent indentation indexof indicate indicates indicating info info: informational injecting injection inline input inside instance instance, instance: instances instantiates instead intended intentionally interpreter into is isobject it it's item items iterates its itself javascript jquery, jquery-ized js js` js`, jsdoc json just keep know known last lastclickdata lastclickdata; lastclickdata[name] latter leading leave left length; less let letter letter, letters lib libraries library library, lieu lifecycle like limiting line lines lint list listed listing literals little live loading log logger logger~instance logical logs long looks loosely lot made magic magic] manager manner manual many markdown markup match matches matching matters may md mentally merging message method method's methods minification minifier missing mistakes mode module module-constrained module-system-agnostic modulejs modules more most much multiple must myclass name name, named names names, naming near neat, necessary need needed needle never new newline newlines next no node~console nor normally not notable notes nothing number object objects observable of off often omitted on onclick one ones only only, opening operates operators option optional optionally, or order ordered ordering original other otherwise out outside overridden override own package parameter parameters parent parenthesis parenthesis, part party pass passed path pattern per perform placed please possible post post-end prefer preferred pretty primitive problem problems processentry programmers prohibited project prominent promise promises, property property's prototype provide provided purpose purposes question quick quite quotes rare read reason recommend refer rejection related removed repository repository, require required resolve return returning rewrite root root, root[name] router rules rules][general-rules] run same, sample samplemodule sanity, scope script second see semicolons sentence separate separates server services, shall shorthand should shouldn't show showstrings signifies similar similarly simpler single single-line slice so some some-feature: somefunction something sometimes, sort sorted source sourced space space, spaces spec special specific specifically specifying split standalone standard start statements statements, str strict strict; string stringlist strings structure structures, stuff style such suggest suggested suggested-app-structure supposed switches symbols syntax systems tags template, ternary test test, test; testing tests tests, text thankfully that that's the thecontentwewanttoexport; their them then there there's these they things think this this, those though through throughout thus to together tooling top total touppercase trailing tricky trivial true truly two txt type typecasting typecasts typed typed, typeof types types, typical, typically typing ugly umd un-indented undefined under underscores unless unquoted unsent unshift unused up upper upper; uppercase url us usage use used user uses using usually utilizing value value, values var variable variables variables, very via vs want wary way we we're well were what whatever when where whitespace will wind window window* wish with within without wording work working works would wrap wrapping write writes writing written x y y[name] yaml yaml] you your yourself yui zero
---

These rules extend and override the [general rules][general-rules].

If you are writing Angular code, please refer the [Angular](angular.md) rules under JS.
