...
pageid: lang-js
keywords: !!bool !!~haystack !variable $car $elements 'input' 'mousex' 'mousey' 'something' +amount +data +num -1 0 1 100% 2 3 3rd 4 5 6 80 @class @param @property @return @typedef a abc-manager about above access accesses according achieve add added adds after aim alert aliases all alongside alphabetical alphabetically already also always amd amount an and angular angular-specific annotations anonymous anotherclass~speciallyformattedobject any anything api app apply are args arguments arrange array arrays arrayvar arrow as assign assigned assignments assigns associated asterisks at attached automate avoid away back basic be because becomes been before beginning behavior behavioral between bigger bit bits blank block blocks bodies body boilerplate boolean braces brackets branch branches break browser browsers bus but by call callcount calling calls camelcase can capital capitalization capitalized car care careful carefully case cases cast catch certain chained chaining change charactercount characters check checker checking checks class classes cleanliness closely code codes collection colon column come comma commas comment commented comments compare comparison compelling compulsory concat configuration conflict conform considered consistency consistent console const constants constructor constructors constructs consumed contains content contents context control copy copyproperty correct could count counter coverage create creates css current custom data datatypes date datepicker-directive debuggable declaration declared declaring default define defining dependencies dependency describe describes description descriptions designed desired development different directives directly directories disable display displayable dizzy do documented documenting does don't double-quoted duck each early easier easy ecma eg either element else embedding empty enable encourage encouraged end ending enforced entire entirely enumerated environment error es5 es6 eslint eslintignore eslintrc eslintrc-angular etc even event eventemitter every examine example examples exception exceptions executes expanded expected explain explaining export exports exposed extend factory fake faketestclass faketestdata false falsy far few fid-umd file file's filename files find first fit flags folder folders follow followed foobar for forced forces foreach formatted formatting former four from front function functions general general-rules get gets git global globals goes going grammatically guide handler handling has have haystack header help helps here hoisting however html if illustrate illustration illustrative immediately immediatly in include includes incoming increases increment increments indent indentation indexof indicate indicates indicating info informational injecting injection inline input inside instance instances instantiates instead intended intentionally interpreter into is isobject it it's item items iterates its itself javascript jquery jquery-ized js jsdoc json just keep know known lang-md last lastclickdata latter leading leave left length less let letter letters lib libraries library lieu lifecycle like limiting line lines lint list listed listing literals little live loading log logger logger~instance logical logs long looks loosely lot made magic manager manner manual many markdown markup match matches matching matters may md mentally merging message method method's methods minification minifier missing mistakes mode module module-constrained module-system-agnostic modulejs modules more most much multiple must myclass name named names naming nan near neat necessary need needed needle never new newline newlines next no node~console nor normally not notable notes nothing null number object objects observable of off often omitted on onclick one ones only opening opentoken operates operators option optional optionally or order ordered ordering original other otherwise out outside overridden override own package parameter parameters parent parenthesis part party pass passed path pattern per perform placed please possible post post-end prefer preferred pretty primitive problem problems processentry programmers prohibited project prominent promise promises property property's prototype provide provided purpose purposes question quick quite quotes rare read reason recommend refer rejection related removed repository require required resolve response-template-service response-template-service-declined restify return returning rewrite root router rules run same sample samplemodule sanity scope script second see semicolons sentence separate separates server services shall shorthand should shouldn't show showstrings signifies similar similarly simpler single single-line slice so some some-directive-service some-feature somefunction something sometimes sort sorted source sourced space spaces spec special specific specifically specifying split standalone standard start statements str strict string stringlist strings structure structures stuff style such suggest suggested suggested-app-structure supposed switch switches symbols syntax systems tags template ternary test testing tests text thankfully that that's the thecontentwewanttoexport their them then there there's these they things think this those though through throughout thus to together tooling top topic-git total touppercase trailing tricky trivial true truly two txt type typecasting typecasts typed typeof types typical typically typing ugly umd un-indented undefined under underscores unless unquoted unsent unshift unused up upper uppercase url us usage use used user uses using usually utilizing value values var variable variables very via vs want wary way we we're well were what whatever when where whitespace will wind window wish with within without wording work working works would wrap wrapping write writes writing written x xxxx y yaml you your yourself yui zero
---
